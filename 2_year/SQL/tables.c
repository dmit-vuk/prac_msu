...
static enum Errors GetCurrentRecord (struct TableType * tabHandle);
static enum Errors PutCurrentRecord (struct TableType * tabHandle);
static enum Errors PutNewRecord (struct TableType * tabHandle,
        long position, struct links * links);
static enum Errors ModifyLinks (struct TableType *  tabHandle,
        long position, long value, enum Direction dir);
static enum Errors GetInsertPos (struct TableType * tabHandle, long * position);
static enum Errors GetLinks (struct TableType * tabHandle,
        long position, struct links * links);
static enum Errors WriteHeader (struct TableType * tabHandle);
static enum Errors ReadHeader (struct TableType * tabHandle);
static struct TableType * CreateTableHandle (struct TableStruct * tableStruct);
static struct TableType * AllocateTableHandle (void);
static void DeleteTableHandle (struct TableType * tabHandle);
static enum Errors AllocateBuffers (struct TableType * tabHandle);
static void DeallocateBuffers (struct TableType * tabHandle);
static enum Errors AddRecToDeleted (struct TableType * tabHandle);
...
enum Errors createNew(THandle tableHandle)
{
    ...
    if (!tableHandle) return BadHandle;
    for (i = 0; i < tableHandle->tableInfo.fieldNumber; i++) {
        memset(tableHandle->pFieldStruct[i].pNewValue, 0,
...
enum Errors insertNew(THandle tableHandle)
{
    ...
    retval = ModifyLinks (tableHandle,
            tableHandle->currentPos, position, LINK_PREV);
    if (retval != OK) return retval;
    retval = ModifyLinks (tableHandle,
            tableHandle->links.prevOffset, position, LINK_NEXT);
    if (retval != OK) return retval;
...
enum Errors insertaNew(THandle tableHandle)
{
    ...
    if (retval != OK) return retval;
    retval = ModifyLinks (tableHandle,
            tableHandle->tableInfo.firstRecordOffset, position, LINK_PREV);
    if (retval != OK) return retval;
    if (tableHandle->tableInfo.recordNumber == 0) {
        tableHandle->tableInfo.lastRecordOffset = position;
    }
...
enum Errors insertzNew(THandle tableHandle)
{
    ...
    if (retval != OK) return retval;
    retval = ModifyLinks (tableHandle,
            tableHandle->tableInfo.lastRecordOffset, position, LINK_NEXT);
    if (retval != OK) return retval;
    if (tableHandle->tableInfo.recordNumber == 0) {
        tableHandle->tableInfo.firstRecordOffset = position;
    }
...
enum Errors getFieldName(THandle tableHandle, unsigned index, const char **pFieldName)
{
    if (!tableHandle) return BadHandle;
...
static enum Errors WriteHeader (struct TableType * tabHandle)
{
    ...
    if (tabHandle->pFieldStruct) {
        WRITE_DATA (tabHandle->pFieldStruct,
                sizeof (FieldStruct) * tabHandle->tableInfo.fieldNumber);
    } else
...
static struct TableType * AllocateTableHandle (void)
{
    struct TableType * handle;

    handle = (struct TableType *) calloc (1, sizeof (struct TableType));
...
static struct TableType * CreateTableHandle (struct TableStruct * tableStruct)
{
    ...
    handle->pFieldStruct = (FieldStruct *) calloc
            (tableStruct->numOfFields, sizeof (FieldStruct));
    if (!handle->pFieldStruct) {
        DeleteTableHandle (handle);
        return NULL;
    }
    for (i = 0; i < tableStruct->numOfFields; i++) {
        strcpy(handle->pFieldStruct[i].fieldName, (const char *)tableStruct->fieldsDef[i].name);
    ...
    handle->tableInfo.dataOffset = sizeof (FILE_SIGNATURE) + sizeof
        (struct TableInfo) +handle->tableInfo.fieldNumber * sizeof (FieldStruct);
...
static enum Errors AllocateBuffers (struct TableType * tabHandle)
{
    ...
    for (i = 0; i < tabHandle->tableInfo.fieldNumber; i++) {
        tabHandle->pFieldStruct[i].pNewValue =
                (char *)malloc(tabHandle->pFieldStruct[i].len);
...
static enum Errors ReadHeader (struct TableType * tabHandle)
{
    ...
    tabHandle->pFieldStruct = (FieldStruct *)calloc
            (tabHandle->tableInfo.fieldNumber, sizeof (FieldStruct));
    if (!tabHandle->pFieldStruct) return CantReadData;
    READ_DATA (tabHandle->pFieldStruct, sizeof
            (FieldStruct) * tabHandle->tableInfo.fieldNumber);
    return OK;
}
...
static enum Errors GetInsertPos (struct TableType * tabHandle, long * position)
{
    ...
    if (tabHandle->tableInfo.firstDeletedOffset >=
            tabHandle->tableInfo.dataOffset) {
        ...
        *position = tabHandle->tableInfo.dataOffset +
...
static enum Errors AddRecToDeleted (struct TableType * tabHandle)
{
    ...
    WRITE_DATA(&tabHandle->tableInfo.firstDeletedOffset,
            sizeof(tabHandle->tableInfo.firstDeletedOffset));
    ...
    if (tabHandle->links.prevOffset == -1) { // first record is deleted
        ...
        MOVE_POS(tabHandle->links.prevOffset +
                sizeof(tabHandle->links.prevOffset));
        WRITE_DATA(&tabHandle->links.nextOffset,
                sizeof(tabHandle->links.nextOffset));
    }
    if (tabHandle->links.nextOffset == 0) { // last record is deleted
        ...
        WRITE_DATA(&tabHandle->links.prevOffset,
                sizeof(tabHandle->links.prevOffset));
    }
    return OK;
}
